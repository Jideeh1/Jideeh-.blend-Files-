...
#
# >>> SET THE CHARACTER NAME HERE <<<  (used for the final rig + collection name)
charname = "Character "

# Set True to hide HairShadow/_FX helper objects. Left False so nothing
# (including geonode-driven outline/effect meshes) disappears from your file.
...
# --- Bone_Root / Bip001 armature finder --------------------------------------
# This skeleton's armature is named "Bone_Root" and is often parented to an
# Empty named after the character. Grab the character armature, and if it hangs
# off an Empty, adopt that Empty's name and delete it (matches the original ZZZ
# workflow this skeleton was built for).
faceobj = None
arm = None
...
if arm is None:
    raise Exception("No character armature found. Expected something like 'Bone_Root'.")

# Prefer an armature literally named "Bone_Root" if one exists.
_exact = bpy.data.objects.get("Bone_Root")
if _exact is not None and _exact.type == 'ARMATURE':
    arm = _exact

# If Bone_Root is parented to an Empty, use that Empty's name and remove it.
if arm.parent is not None and arm.parent.type == 'EMPTY':
    parent_empty = arm.parent
    if charname.strip() in ("", "Character"):
        charname = parent_empty.name + " "
    arm.parent = None
    try:
...
bpy.ops.object.select_all(action='DESELECT')
# NOTE: removed the original shader-material delete + orphans_purge(do_recursive)
# from here. That purge destroys any material / geometry-node group not actively
# assigned at that instant. This is a rig-only build, so it purges nothing.