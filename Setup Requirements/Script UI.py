...
CAMRIG_NAME = "CAM_RIG"
CAMRIG_WGT_COLLECTION = "WGT_CAM_RIG"
CAMRIG_CON_AIM = "CamRig Aim"
CAMRIG_CON_FACE = "CamRig Face"
CAMRIG_CON_SNAP = "CamRig Snap"
CAMRIG_PROP_TRACK = "Track To"
CAMRIG_PROP_DOF = "Depth of Field"
CAMRIG_AIM_DIST = 5.0
CAMRIG_YELLOW = (1.0, 0.82, 0.06)
CAMRIG_MCH_BONES = ("MCH_TRACK", "MCH_ROLL")
...
def _camrig_widget_collection(scene):
    coll = bpy.data.collections.get(CAMRIG_WGT_COLLECTION)
    if coll is None:
        coll = bpy.data.collections.new(CAMRIG_WGT_COLLECTION)
...
def _camrig_mesh_object(name, verts, edges, faces, coll):
    _camrig_purge_object(name)
...
def _camrig_thicken(verts, edges, faces, depth):
    if not faces:
        return verts, edges, faces
    count_verts = len(verts)
    ...
    out_faces += [[i + count_verts for i in reversed(f)] for f in faces]
    for f in faces:
        total = len(f)
        for k in range(total):
            a = f[k]
            ...
            key = (a, b) if a < b else (b, a)
            if tally[key] == 1:
                out_faces.append([b, a, a + count_verts, b + count_verts])
    return out, [tuple(e) for e in edges], out_faces

def _camrig_bracket_frame(hw, hh, arm, th):
    verts = []
    faces = []
...
def _camrig_ring(radius, th, seg):
    verts = []
...
def _camrig_rect_ring(hw, hh, th):
...
def _camrig_root_shape(scale):
    verts = [(v[0] * scale, v[1] * scale, v[2] * scale) for v in CAMRIG_ROOT_VERTS]
    return verts, [tuple(e) for e in CAMRIG_ROOT_EDGES], []

def _camrig_set_bone_color(pb, palette, rgb=None):
...
def _camrig_find_camera(context):
    scene = context.scene
...
def _camrig_frame_extents(cam, scene, dist):
    cd = cam.data
...
def _camrig_clear_drivers(id_data, path, index=-1):
...
def _camrig_purge_constraint_drivers(ob):
...
def _camrig_make_driver(id_data, path, entries=(), dist_entries=(), coeffs=(0.0, 1.0)):
    path, index = _camrig_split_index(path)
    _camrig_clear_drivers(id_data, path, index)
...
def _camrig_distance_scale(rig, bone_name, base):
    for axis in range(3):
        _camrig_make_driver(
            rig,
...
def build_cam_rig(context):
    ...
    cam = _camrig_find_camera(context)
    cd = cam.data

    _camrig_purge_object(CAMRIG_NAME)
    if cam.parent is not None and cam.parent.name == CAMRIG_NAME:
        cam.parent = None

    coll = _camrig_widget_collection(scene)
    ...
    aim_point = loc + fwd * CAMRIG_AIM_DIST

    hw, hh = _camrig_frame_extents(cam, scene, CAMRIG_AIM_DIST)
    span = min(hw, hh)
    seg = CAMRIG_AIM_DIST * 0.14
    depth = span * 0.055

    wgt_frame = _camrig_mesh_object(
        "WGT_CamFrame",
        *_camrig_thicken(*_camrig_bracket_frame(hw, hh, span * 0.38, span * 0.10), depth),
        coll)
    wgt_focus = _camrig_mesh_object(
        "WGT_CamFocus",
        *_camrig_thicken(*_camrig_ring(span * 0.26, span * 0.055, 32), depth),
        coll)
    wgt_body = _camrig_mesh_object(
        "WGT_CamBody",
        *_camrig_thicken(*_camrig_rect_ring(span * 0.22, span * 0.16, span * 0.035), depth * 0.8),
        coll)
    wgt_root = _camrig_mesh_object(
        "WGT_CamRoot",
        *_camrig_root_shape(max(CAMRIG_AIM_DIST * 0.22, 0.4)),
        coll)

    arm_data = bpy.data.armatures.new(CAMRIG_NAME)
    rig = bpy.data.objects.new(CAMRIG_NAME, arm_data)
    ...
    root = ebs.new("ROOT")
    root.head = ground
    root.tail = ground + Vector((0.0, max(CAMRIG_AIM_DIST * 0.2, 0.4), 0.0))
    ...
    shapes = (
        ...
        ("AIM", wgt_frame, "CUSTOM", CAMRIG_YELLOW),
        ("FOCUS", wgt_focus, "CUSTOM", CAMRIG_YELLOW),
    )
    for bone_name, shape, palette, rgb in shapes:
        pb = rig.pose.bones[bone_name]
        pb.custom_shape = shape
        pb.use_custom_shape_bone_size = False
        _camrig_set_bone_color(pb, palette, rgb)

    for bone_name in CAMRIG_MCH_BONES:
    ...
    con_snap.name = CAMRIG_CON_SNAP
    con_snap.target = rig
    con_snap.subtarget = "AIM"
    con_snap.track_axis = "TRACK_NEGATIVE_Z"

    cam[CAMRIG_PROP_TRACK] = True
    cam[CAMRIG_PROP_DOF] = True
    descriptions = (
        (CAMRIG_PROP_TRACK, "Aim the camera at the frame control"),
        (CAMRIG_PROP_DOF, "Depth of field focused on the focus control"),
    )
    ...
    _camrig_make_driver(cd, "dof.use_dof", entries=(("dof", cam, '["%s"]' % CAMRIG_PROP_DOF),))
    _camrig_make_driver(
        rig,
        'pose.bones["MCH_TRACK"].constraints["%s"].influence' % CAMRIG_CON_AIM,
        entries=(("trk", cam, '["%s"]' % CAMRIG_PROP_TRACK),),
    )
    _camrig_make_driver(
        cam,
        'constraints["%s"].influence' % CAMRIG_CON_SNAP,
        entries=(("trk", cam, '["%s"]' % CAMRIG_PROP_TRACK),),
    )
    _camrig_make_driver(
        rig,
    ...
    _camrig_distance_scale(rig, "AIM", CAMRIG_AIM_DIST)
    _camrig_distance_scale(rig, "FOCUS", CAMRIG_AIM_DIST)
...
class JIDEEH_OT_build_cam_rig(bpy.types.Operator):
    bl_idname = "jideeh.build_cam_rig"
    bl_label = "Camrig"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        try:
            disable_auto_keying()
            rig, cam = build_cam_rig(context)
            disable_auto_keying()
            self.report({"INFO"}, f'Built "{rig.name}" on "{cam.name}".')
            return {"FINISHED"}
        except Exception as error:
            self.report({"ERROR"}, str(error))
            raise
...
class JIDEEH_PT_script_runner_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("jideeh.build_cam_rig", text="Camrig")
