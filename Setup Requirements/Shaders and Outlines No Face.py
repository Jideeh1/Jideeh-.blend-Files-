...
# Run this script
#
# ### NO-RIG / NO-FACE VARIANT ###
# 1) The Rigify/Expykit armature generation has been removed. This does shader,
...
#
# 2) Models with no face mesh (weapons, props, many enemies/bosses) are handled.
#    The script detects this and skips eye shadow, eye transparency, eyebrow
#    joining, eye highlights and the face solidify outlines. Body/hair/weapon
#    shading and outlines still run normally. A missing Body_1 mesh is handled
#    too, and a missing texture set no longer aborts the whole run.
...
faceobj = None   # stays None on models with no face mesh (weapons, props, many enemies)
for obj in bpy.data.objects:
...
if faceobj is not None:
    ...
    print("\n\nNo face mesh found - skipping eye shadow / eye transparency setup.\n\n")
mats = []
outs = []
...
def geonode(arm):
    ...
    # Find the main body object to hang the outline-thickness drivers off.
    bod = None
    for obj in meshes:
    ...
    if bod is None:
        # No body mesh at all (weapon-only / prop / some enemy models).
        # Promote the first non-face mesh to driver master so the outline
        # thickness is still controllable from one place.
        for obj in meshes:
            if "face" not in obj.name.lower():
                bod = obj
                break
        if bod is not None:
            print("No Body_1 mesh; using", bod.name, "as the outline driver master.")

    if bod is not None:
        mod = bod.modifiers.new("Extra FX", "NODES")
        mod.node_group = bpy.data.node_groups["Extra FX Geonode"] # order matters; fx geonode needs to be above outline.
        mod["Socket_0_attribute_name"] = "cast shadow"
        mod["Socket_11_attribute_name"] = "shadowsharpness"

        mod = bod.modifiers.new("Outlines", "NODES")
        mod.node_group = grp
        modassign(mod)

    for ob in meshes: # Assign outlines modifier to the remaining meshes
        if ob == bod:
            continue # already did this.
        mod = ob.modifiers.new("Extra FX", "NODES")
        mod.node_group = bpy.data.node_groups["Extra FX Geonode"]
        mod["Socket_0_attribute_name"] = "cast shadow"
        mod["Socket_11_attribute_name"] = "shadowsharpness"
    ...
    # Cast shadow drivers. The face is normally the master; without one, fall
    # back to the body/first mesh so the values still sync across bodyparts.
    master = faceobj if faceobj is not None else bod
    if master is not None:
        for ob in meshes:
            if ob == master:
                continue
            syncdriver(ob, "Socket_1", master, 'modifiers["Extra FX"]["Socket_1"]') # cast shadow
            syncdriver(ob, "Socket_10", master, 'modifiers["Extra FX"]["Socket_10"]') # shadow sharpness
    ...
    for mat in bpy.data.materials:
        if "Outlines" in mat.name:
            try:
                ...
                print("Outline textures skipped for", mat.name, "-", e)
...
# Everything in this block only applies to models that actually have a face mesh.
...
if faceobj is not None:
    ...
    for ob in arm.children:
        if ob.name.lower().endswith("_eyebrow"):
            eyebrowobj = ob
    ...
    # NOTE: the rig section was what renamed these groups to SknHighlights.L/.R,
    # so without a rig they may still have their raw names.
...
if faceoutobj is not None:
    ...
    print("No face mesh - skipped face solidify outlines and face outline material slots.")
